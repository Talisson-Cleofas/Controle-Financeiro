...
r.post('/checkout',auth,async(req,res)=>{
  ...
  const data=await mp('/checkout/preferences',{method:'POST',headers:{'X-Idempotency-Key':crypto.randomUUID()},body:JSON.stringify({items:[{id:`cf-${plan.id}`,title:`Controle Financeiro - Plano ${plan.name}`,quantity:1,currency_id:'BRL',unit_price:plan.price}],payer:{email:req.user.email},external_reference:reference,back_urls:{success:`${frontend}/vendas?payment=success`,pending:`${frontend}/vendas?payment=pending`,failure:`${frontend}/vendas?payment=failure`},auto_return:'approved',notification_url:`${backend}/api/billing/webhook`,payment_methods:{excluded_payment_types:[{id:'ticket'}]},statement_descriptor:'CONTROLE FIN'})});
