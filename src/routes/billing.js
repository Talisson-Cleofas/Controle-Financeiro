...
async function mp(path,options={}){
  ...
  if(!response.ok){const apiErrors=Array.isArray(data.errors)?data.errors.map(e=>[e?.code,e?.message,e?.details,e?.property].filter(Boolean).join(': ')||JSON.stringify(e)).filter(Boolean).join('; '):'';const message=[data.message,apiErrors].filter(Boolean).join(' — ')||'Falha na comunicação com o Mercado Pago.';throw Object.assign(new Error(message),{status:502,details:data});}
  return data;
}
...
r.post('/checkout',auth,async(req,res)=>{
  ...
  await Payment.create({userId:req.user._id,preferenceId:data.id,status:'pending',plan:plan.id,amount:plan.price});const checkoutUrl=mercadoPagoTestMode()?(data.sandbox_init_point||data.init_point):(data.init_point||data.sandbox_init_point);res.json({checkoutUrl,preferenceId:data.id});
});
